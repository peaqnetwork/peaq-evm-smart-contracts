...
contract GasStationFactory {
    ...
    address public fundingToken; // The contract address of the token used in funding machine smart account so it can pay storage deposit
    ...
    constructor(address _owner, address _gasStation, address _fundingToken) {
    ...
    function deployNewContract(
        ...
        // verify that the machine owner approves this tx
        bytes32 eoaMessageHash = keccak256(
            abi.encodePacked(address(this), eoa, nonce)
        );

        require(
            _verifySignature(eoaMessageHash, eoaSignature, nonce),
    ...
    function transferGasStationBalance(
        ...
        uint256 gasStationBalance = IERC20(fundingToken).balanceOf(
            address(this)
        );
        ...
        bool success = IERC20(fundingToken).transfer(
    ...
    /**
     * @dev Execute a meta-transaction via the gas station factory contract.
     * @param eoa The user (machine owner) on whose behalf the transaction is executed.
    ...
    function executeMetaTransaction(
        ...
        if (
            ...
            uint256 machineBalance = IERC20(fundingToken).balanceOf(
                machineAddress
            );
            ...
            if (machineBalance <= MIN_BALANCE) {
                ...
                IERC20(fundingToken).transfer(machineAddress, FUNDING_AMOUNT);
            }
        }
